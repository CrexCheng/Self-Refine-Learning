...
class ChatLLM:
    ...
    def delete_last_cache_item(self):
        with open(self.cache_path, "r+", encoding="utf-8") as f:  # 读写模式打开文件
    ...
    def chat_completion(self, messages, use_cache: bool = True, **kwargs):
        ...
        if self.model_name in self.openai_model_name:
            response = self.__azure_chat_completion(messages, **kwargs)
        elif self.model_name in os.getenv("DEEPSEEK_MODEL_NAME"):
            response = self.__deepseek_chat_completion(messages)
        elif self.model_name in os.getenv("LLAMA_MODEL_NAME"):
            response = self.__llama_chat_completion(messages)
        elif self.model_name in os.getenv("QWEN_MODEL_NAME"):
    ...
    def __azure_chat_completion(self, messages, **kwargs):
        """
    ...
    def __deepseek_chat_completion(self, messages, **kwargs):
        """
